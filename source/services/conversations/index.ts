...
import { provider as conversations } from '@/providers/data/conversations'
...
const create = async (
	...
	try {
		const conversation = await conversations.create({ ...request.body, id: generateId() })
 ...
 */
const update = async (
	request: ServiceRequest<UpdateConversationPayload, { conversationId: string }>
