...
const find = async (
	...
	try {
		...
		if (!request.user?.isGroot)
			query.push({ field: 'participants', operator: 'includes', value: request.user!.id })
...
const update = async (
	...
	try {
		const group = await groups.update({ ...request.body, id: request.params.groupId })
...
const join = async (
	...
	try {
		...
		if (foundGroups.length === 0 || !foundGroups[0])
			throw new ServerError('entity-not-found', 'Could not find a group with that code.')
