...
class ConversationProvider implements DataProvider<Conversation> {
	...
	async find(queries: Array<Query<Conversation>>): Promise<Conversation[]> {
		...
		// Build the query
		logger.silly('[firebase/conversations/find] parsing query - %s', stringify(queries))
		...
		for (const doc of docs) {
			...
			const data = doc.data()
			if (!doc.exists || !data) {
				logger.silly('[firebase/conversations/find] received empty doc - discarding')
				continue
			}
		...
		logger.info('[firebase/conversations/find] returning list of found conversations')
	...
	async create(data: Conversation): Promise<Conversation> {
		logger.info('[firebase/conversations/create] create conversation %s', data.id)
		...
		try {
			...
			logger.info('[firebase/conversations/create] successfully created conversation')
			return data
		} catch (error: unknown) {
	...
	async update(data: Partial<Conversation>): Promise<Conversation> {
		logger.info('[firebase/conversations/update] updating conversation %s', data.id)
	...
	async delete(id: string): Promise<void> {
		...
		try {
			...
			logger.info('[firebase/conversations/delete] sucessfully deleted conversation')
		} catch (caughtError: unknown) {
			const error = caughtError as FirebaseError
			logger.warn(
